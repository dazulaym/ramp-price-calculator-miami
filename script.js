...
const ACCESSORY_PRODUCTS = {
  aluminum: [
    { model: "12 x 32", detail: '1 3/8"–2" rise', purchase: 180, rental: 50 },
    { model: "12 x 36", detail: '1 3/8"–2" rise', purchase: 180, rental: 50 },
    { model: "24 x 32", detail: '2 3/8"–3 1/2" rise', purchase: 180, rental: 50 },
    { model: "24 x 36", detail: '2 3/8"–3 1/2" rise', purchase: 260, rental: 50 },
    { model: "36 x 32", detail: '2 1/2"–6" rise', purchase: 270, rental: 50 },
    { model: "36 x 36", detail: '2 1/2"–6" rise', purchase: 420, rental: 50 },
    { model: "48 x 32", detail: '2 1/2"–6" rise', purchase: 430, rental: 50 },
    { model: "48 x 36", detail: '2 1/2"–6" rise', purchase: 475, rental: 50 }
  ],
  rubber: [
    { model: '1/2"', detail: "", purchase: 90, rental: null },
    { model: '3/4"', detail: "", purchase: 170, rental: null },
    { model: '1"', detail: "", purchase: 200, rental: null },
    { model: '1 1/4"', detail: "", purchase: 330, rental: null },
    { model: '1 1/2"', detail: "", purchase: 360, rental: null },
    { model: '2"', detail: "", purchase: 550, rental: null }
  ],
  suitcase: [
    { model: "2 ft", detail: '4" max rise', purchase: 230, rental: 75 },
    { model: "3 ft", detail: '6" max rise', purchase: 230, rental: 75 },
    { model: "4 ft", detail: '8" max rise', purchase: 280, rental: 75 },
    { model: "5 ft", detail: '10" max rise', purchase: 350, rental: 75 },
    { model: "6 ft", detail: '12" max rise', purchase: 440, rental: 75 },
    { model: "7 ft", detail: '14" max rise', purchase: 490, rental: 75 },
    { model: "8 ft", detail: '16" max rise', purchase: 600, rental: 75 },
    { model: "10 ft", detail: '20" max rise', purchase: 670, rental: 100 }
  ]
};

const ACCESSORY_CATEGORY_NAMES = {
  aluminum: "Aluminum Threshold Ramp",
  rubber: "Rubber Threshold Ramp",
  suitcase: "Suitcase Ramp"
};
...
function addAccessoryRow(category, initial = {}) {
  const container = $(`${category}AccessoryRows`);
  const products = ACCESSORY_PRODUCTS[category];
  const row = document.createElement("div");
  row.className = "accessory-row";
  row.dataset.category = category;

  const options = products.map((product, index) => {
    const detail = product.detail ? ` — ${product.detail}` : "";
    return `<option value="${index}">${product.model}${detail}</option>`;
  }).join("");

  row.innerHTML = `
    <label><span>Model</span><select class="accessory-model"><option value="">Select model</option>${options}</select></label>
    <label><span>Quantity</span><input class="accessory-quantity" type="number" min="1" step="1" value="${initial.quantity || 1}"></label>
    <div class="accessory-price">$0.00</div>
    <button class="remove-accessory-button" type="button" aria-label="Remove product">×</button>`;

  if (initial.productIndex !== undefined) row.querySelector("select").value = String(initial.productIndex);
  row.querySelector("select").addEventListener("change", calculate);
...
function selectedAccessories() {
  ...
  document.querySelectorAll(".accessory-row").forEach(row => {
    ...
    const indexValue = row.querySelector("select").value;
    if (indexValue === "") return;
    const quantity = Math.max(1, Math.floor(Number(row.querySelector("input").value) || 1));
    const product = ACCESSORY_PRODUCTS[category][Number(indexValue)];
    items.push({ category, categoryName: ACCESSORY_CATEGORY_NAMES[category], ...product, quantity,
      purchaseTotal: product.purchase * quantity,
      rentalMonthlyTotal: product.rental == null ? 0 : product.rental * quantity });
  });
  return items;
}

function calculateAccessoryTotals() {
  const items = selectedAccessories();
  const purchaseTotal = items.reduce((sum, item) => sum + item.purchaseTotal, 0);
  const rentalMonthlyTotal = items.reduce((sum, item) => sum + item.rentalMonthlyTotal, 0);
  const rubberPurchaseTotal = items.filter(item => item.category === "rubber").reduce((sum, item) => sum + item.purchaseTotal, 0);
  return { items, purchaseTotal, rentalMonthlyTotal, rubberPurchaseTotal };
}

function updateAccessoryRowPrices(quoteType) {
  document.querySelectorAll(".accessory-row").forEach(row => {
    const category = row.dataset.category;
    const indexValue = row.querySelector("select").value;
    const price = row.querySelector(".accessory-price");
    if (indexValue === "") { price.textContent = "$0.00"; return; }
    const product = ACCESSORY_PRODUCTS[category][Number(indexValue)];
    const qty = Math.max(1, Math.floor(Number(row.querySelector("input").value) || 1));
    if (quoteType === "rental" && product.rental != null) price.textContent = `${money(product.rental * qty)}/mo`;
    else price.textContent = money(product.purchase * qty);
  });
}
...
function calculate() {
  ...
  const accessories = calculateAccessoryTotals();

  const squareFeet = rampLength * 3 + p55 * 25 + p54 * 20 + p44 * 16;
  ...
  const purchaseInstallation = squareFeet > 0 ? Math.max(squareFeet * settings.purchaseInstallPerSqft, settings.minimumPurchaseInstall) : 0;

  const newTotal = newRampPrice + purchaseInstallation + accessories.purchaseTotal;
  const usedTotal = usedRampPrice + purchaseInstallation + accessories.purchaseTotal;

  const baseMonthlyRental = squareFeet * settings.monthlyRentalPerSqft;
  const monthlyRental = baseMonthlyRental + accessories.rentalMonthlyTotal;
  const rentalInstallation = squareFeet > 0 ? Math.max(squareFeet * settings.rentalInstallPerSqft, settings.minimumRentalInstall) : 0;
  const rentalRemoval = squareFeet > 0 ? Math.max(squareFeet * settings.rentalRemovalPerSqft, settings.minimumRentalRemoval) : 0;
  const rentalAccessoriesFirst = accessories.rentalMonthlyTotal * 3 + accessories.rubberPurchaseTotal;
  const rentalFirstPayment = baseMonthlyRental * 3 + rentalInstallation + rentalRemoval + rentalAccessoriesFirst;

  const results = { rampLength, p55, p54, p44, squareFeet, accessories,
    newRampPrice, purchaseInstallation, newTotal, usedRate, usedRampPrice, usedTotal,
    baseMonthlyRental, monthlyRental, rentalInstallation, rentalRemoval, rentalAccessoriesFirst, rentalFirstPayment };
...
function getSelectedQuote(results) {
  ...
  if (type === "used") return { type, name: "Used Ramp Purchase", total: results.usedTotal,
    lines: [["Used Ramp Price", results.usedRampPrice], ["Installation", results.purchaseInstallation], ["Additional Products", results.accessories.purchaseTotal]] };
  if (type === "rental") return { type, name: "Ramp Rental", total: results.rentalFirstPayment,
    lines: [["First 3 Months — Modular Ramp", results.baseMonthlyRental * 3], ["Installation", results.rentalInstallation], ["Removal", results.rentalRemoval], ["Additional Products", results.rentalAccessoriesFirst]],
    ongoing: results.monthlyRental };
  return { type: "new", name: "New Ramp Purchase", total: results.newTotal,
    lines: [["New Ramp Price", results.newRampPrice], ["Installation", results.purchaseInstallation], ["Additional Products", results.accessories.purchaseTotal]] };
}
...
function updateScreen(results) {
  ...
  $("newAccessories").textContent = money(results.accessories.purchaseTotal);
  ...
  $("usedAccessories").textContent = money(results.accessories.purchaseTotal);
  ...
  $("rentalAccessoriesFirst").textContent = money(results.rentalAccessoriesFirst);
  $("rentalFirstPayment").textContent = money(results.rentalFirstPayment);
  $("ongoingRental").textContent = money(results.monthlyRental);
  $("accessoryPurchaseTotal").textContent = money(results.accessories.purchaseTotal);
  $("accessoryRentalMonthly").textContent = money(results.accessories.rentalMonthlyTotal);
  updateAccessoryRowPrices(selectedType());
...
function quoteSummaryText() {
  ...
  const accessoryLines = results.accessories.items.length ? "\nAdditional Products:\n" + results.accessories.items.map(item => `- ${item.quantity} x ${item.categoryName} ${item.model}: ${quote.type === "rental" && item.rental != null ? money(item.rentalMonthlyTotal) + "/month" : money(item.purchaseTotal)}`).join("\n") : "";
...
- Remaining Balance: ${money(balance)}${ongoing}${accessoryLines}
...
function generatePdf() {
  ...
  if (logoData) {
    ...
    doc.roundedRect(left, 12, 158, 44, 5, 5, "F");
    doc.addImage(logoData, "PNG", left + 6, 20, 146, 28);
  } else {
  ...
  const configRows = [
    [`Ramp Length: ${results.rampLength} linear ft`, `5 x 5 Platforms: ${results.p55}`],
    [`5 x 4 Platforms: ${results.p54}`, `4 x 4 Platforms: ${results.p44}`],
  ...
  configRows.forEach((row, index) => {
    ...
    if (row[1]) doc.text(row[1], 454, rowY);
  });
  ...
  const cards = [
    {
      ...
      rows: [
        ...
        ["Products", results.accessories.purchaseTotal]
      ],
      totalLabel: "TOTAL",
    ...
    },
    {
      ...
      title: "RENTAL",
      rows: [
        ["First 3 Months — Modular Ramp", results.baseMonthlyRental * 3],
        ["Installation", results.rentalInstallation],
        ["Removal", results.rentalRemoval],
        ["Additional Products", results.rentalAccessoriesFirst]
      ],
  ...
  if (results.accessories.items.length) {
    y += cardHeight + 12;
    doc.setTextColor(...charcoal);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(8.5);
    doc.text("ADDITIONAL PRODUCTS", left, y);
    doc.setDrawColor(...green);
    doc.line(left, y + 4, right, y + 4);
    y += 14;
    doc.setFontSize(6.8);
    results.accessories.items.forEach(item => {
      const pricing = selected === "rental" && item.rental != null
        ? `${money(item.rentalMonthlyTotal)}/month`
        : `${money(item.purchaseTotal)} purchase`;
      doc.setFont("helvetica", "normal");
      doc.text(`${item.quantity} x ${item.categoryName} — ${item.model}${item.detail ? ` (${item.detail})` : ""}`, left, y);
      doc.setFont("helvetica", "bold");
      doc.text(pricing, right, y, { align: "right" });
      y += 10;
    });
    y += 4;
  } else {
  ...
  // Deposit for the option selected in the calculator
  const selectedQuote = getSelectedQuote(results);
  const selectedDeposit = selectedQuote.total * settings.depositPercent / 100;
  const selectedBalance = selectedQuote.total - selectedDeposit;

  doc.setFillColor(...green);
  doc.roundedRect(left, y, right - left, 54, 6, 6, "F");
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(7.6);
  doc.text(`Selected option: ${selectedQuote.name}`, left + 14, y + 36);

  doc.setFont("helvetica", "bold");
  doc.setFontSize(14);
  doc.text(money(selectedDeposit), right - 14, y + 22, { align: "right" });

  doc.setFontSize(7.6);
  doc.text(
    `Remaining balance: ${money(selectedBalance)}`,
    right - 14,
    y + 39,
    { align: "right" }
  );
...
function resetForm() {
  ...
  document.querySelectorAll(".accessory-rows").forEach(container => container.innerHTML = "");
  ["aluminum", "rubber", "suitcase"].forEach(category => addAccessoryRow(category));
...
document.querySelectorAll("[data-add-category]").forEach(button => button.addEventListener("click", () => addAccessoryRow(button.dataset.addCategory)));
