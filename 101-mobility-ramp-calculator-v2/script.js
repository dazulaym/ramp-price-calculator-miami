...
async function generatePdf() {
  ...
  info.forEach(([label, value]) => {
    ...
    doc.setFont("helvetica", "normal");
    const wrapped = doc.splitTextToSize(value, 370);
  ...
  doc.setFont("helvetica", "bold");
  doc.text(`Total Square Feet: ${results.squareFeet.toLocaleString("en-US", {maximumFractionDigits: 2})}`, 250, y);

  y += 34;
  doc.setFont("helvetica", "bold");
  doc.setFontSize(11);
  doc.text("SELECTED QUOTE", left, y);
  doc.line(left, y + 6, right, y + 6);
  y += 28;

  doc.setFillColor(245, 247, 242);
  doc.roundedRect(left, y - 14, right - left, 32 + quote.lines.length * 22 + (quote.type === "rental" ? 20 : 0), 6, 6, "F");
  doc.setFont("helvetica", "bold");
  doc.setFontSize(12);
  doc.text(quote.name, left + 12, y + 4);
  y += 28;

  doc.setFontSize(10);
  quote.lines.forEach(([label, value]) => {
    doc.setFont("helvetica", "normal");
    doc.text(label, left + 12, y);
    doc.setFont("helvetica", "bold");
    doc.text(money(value), right - 12, y, { align: "right" });
    y += 22;
  });

  doc.setDrawColor(210, 214, 216);
  doc.line(left + 12, y - 8, right - 12, y - 8);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(12);
  doc.text("Quote Total", left + 12, y + 5);
  doc.setTextColor(...green);
  doc.text(money(quote.total), right - 12, y + 5, { align: "right" });
  doc.setTextColor(...charcoal);
  y += 28;
  ...
  if (quote.type === "rental") {
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    doc.text(`Ongoing Monthly Payment beginning Month 4: ${money(quote.ongoing)}`, left + 12, y);
    y += 22;
  }
  ...
  doc.roundedRect(left, y, right - left, 60, 6, 6, "F");
  doc.setTextColor(...charcoal);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(12);
  doc.text(`${settings.depositPercent}% DEPOSIT REQUIRED TO APPROVE AND SCHEDULE`, left + 14, y + 23);
  doc.setFontSize(16);
  doc.text(money(deposit), right - 14, y + 25, { align: "right" });
  doc.setFontSize(10);
  doc.text(`Remaining balance: ${money(balance)}`, left + 14, y + 45);
  y += 84;
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  const approval = `I approve the selected quote and authorize 101 Mobility of Miami to proceed. I understand that a ${settings.depositPercent}% deposit in the amount of ${money(deposit)} is required to approve the quote and begin scheduling or ordering. Final installation is subject to site verification and the terms stated in this quote.`;
  ...
  doc.text("This quote is an estimate and is subject to final site verification, product availability, applicable permits, and written terms.", 306, 758, { align: "center" });
